    ...
    for (var i = 0; i < data.pts.length; i++) {
        ...
        eachdata = {
            ...
            datapoints: [[timestamp,
                data.pts[i].value]],
                // data.pts[i].value + randval]], // debug
            ttl: (data.pts[0].arg ?
...
function decrease_range (range) {
    ...
    if (index > -1) return range_input[index];
...
function query_generator (device_id, range, qtype, endtime) {
    ...
    var q_data = {
        ...
        metrics: [{
            ...
            tags: qtype ? {
                range_tag: decrease_range (range)
            } : {},
            aggregators: qtype ? [{
                name: qtype ? qtype : '',
                sampling: {
...
function get_query(device_id, range) {
    ...
    //     }]
    // }
    var q_data = query_gen(device_id, range);
 ...
 */
function summary(device_id, range, qtype, qtime) {
    var mls = ms(validateRange(range));
    ...
    //     end_absoulte: now,
    // };
    var ave_metric = query_gen (device_id, range, qtype, qtime);
